import pytest
from sw_game.sw_game import JediMaster,SithLord,ForceUser
# SithLord, ForceUser
...
def test_jedi_class_method():
    ...
    expected = 'There is no emotion, there is only PEACE.'
    assert actual == expected
...
def test_force_user_count():
    ...
    JediMaster('Luke')  #1
    SithLord('Darth Vader') #1

    actual = JediMaster.get_count()
    expected = 1
    assert actual == expected # 2 == 1
